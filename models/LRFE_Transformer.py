...
def window_partition(x, window_size):
    ...
    """
    B, H, W, C = x.shape
    # print(f"原特征图 shape of x: {x.shape}")
    x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)
    # print(f"新特征图 shape of x: {x.shape}")
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        # 串联模式2 LR先
        # x = self.weight_factor * self.c_attns(x)
        # B, N, C = x.shape
        ...
        # 仅使用LR模块
        x3 = self.c_attns(x)
        attn = self.weight_factor * x3
        
        # 并联模式
        # x3 = self.c_attns(x)
        # attn = attened_x + self.weight_factor*x3 # 使用完整SERT模块
        
        # 串联模式1 RA先
        # x3 = self.c_attns(attened_x)
        # attn = self.weight_factor * x3
...
class SMSBlock(nn.Module):
    def __init__(self,
        ...
        weight_factor=0.1,memory_blocks=128,down_rank=1,
        drop_path=0.0,
        split_size=1,
        ):
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 512, 8, 8).to('cpu')
    out = model(x)
